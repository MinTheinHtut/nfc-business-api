...
export async function commitCompanyImport(request, response, next) {
  ...
  const validRows = analysis.rows.filter((row) => row.valid); const connection = await pool.getConnection(); let created = 0; let updated = 0;
  try {
    ...
    for (const row of validRows) {
      if (row.action === 'create') {
        ...
        await connection.execute(`INSERT INTO companies (${uniqueFields.join(',')}) VALUES (${uniqueFields.map(() => '?').join(',')})`, uniqueFields.map((field) => row.data[field] ?? null)); created += 1;
    ...
    const result = { created, updated, skipped: analysis.summary.invalidRows, errors: analysis.rows.filter((row) => !row.valid).map((row) => ({ rowNumber: row.rowNumber, companyCode: row.companyCode, errors: row.errors })) };
